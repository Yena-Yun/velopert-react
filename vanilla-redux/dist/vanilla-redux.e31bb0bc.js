...
var btnDecrease = document.querySelector('#decrease'); //액션
  ...
  switch (action.type) {
    case TOGGLE_SWITCH:
      return _objectSpread(_objectSpread({}, state), {}, {
        //전달받은 액션객체의 특정 state 변화시킴
        //여기서는 toggle state가 true인 경우는 false로, false인 경우는 true로 변화시킴
        toggle: !state.toggle
      });

    case INCREASE:
      return _objectSpread(_objectSpread({}, state), {}, {
        // 기존 counter state에 액션에서 전달받은 difference를 추가하여 변화시킴
...
var store = (0, _redux.createStore)(reducer); //render 함수 => state가 업데이트될 때마다 호출됨
...
var render = function render() {
  ...
  if (state.toggle) {
    divToggle.classList.add('active'); //toggle이 false인 경우에는
    //divToggle의 class에서 'active'를 제거
  } else {
